 ...
 */
$title = $values = $units = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = $css_animation = $theme_bar = ''; // tcsn-custom
$output = '';
...
if( $theme_bar ){
	$return_theme_bar = 'theme-progress-bar ';
} else {
	$return_theme_bar = '';
}
// tcsn-custom - ends
...
$class_to_filter = '' . $return_theme_bar . 'vc_progress_bar wpb_content_element'; // tcsn-custom 
...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ? ' <span class="vc_label_units"><span class="tc-progress-counter">' . $line['value'] . '</span>' . $units . '</span>' : '';
	// tcsn-custom - label shifted above starts
	if( $theme_bar == 'yes' ){
	$output .= '<h6 class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . $unit . '</h6>'; 
	}
	// tcsn-custom - label shifted above ends
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	// tcsn-custom - label default starts
	if( $theme_bar != 'yes' ){
	$output .= '<small class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . $unit . '</small>';
	}
	// tcsn-custom - label default ends
	if ( $max_value > 100.00 ) {
